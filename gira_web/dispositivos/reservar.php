...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($hora_inicio < '08:00' || $hora_fin > '21:00' || $hora_inicio >= $hora_fin) {
        ...
        $stmt_check = $conn->prepare("
            SELECT id FROM reservas_dispositivos 
            WHERE dispositivo_id = ? AND estado = 'CONFIRMADA'
            AND ((inicio < ? AND fin > ?) OR (inicio < ? AND fin > ?))
        ");
        $stmt_check->bind_param("issss", $dispositivo_id, $fin_dt, $inicio_dt, $inicio_dt, $fin_dt);
        $stmt_check->execute();
        
        if ($stmt_check->get_result()->num_rows > 0) {
            $mensaje = 'Este dispositivo ya está reservado en ese horario.';
            $tipo_mensaje = 'alert-danger';
        } else {
            // Insertar reserva material/dispositivo
            $stmt_insert = $conn->prepare("INSERT INTO reservas_dispositivos (dispositivo_id, usuario_id, inicio, fin, estado) VALUES (?, ?, ?, ?, 'CONFIRMADA')");
            $stmt_insert->bind_param("iiss", $dispositivo_id, $usuario_id, $inicio_dt, $fin_dt);
            
            if ($stmt_insert->execute()) {
                $mensaje = 'Dispositivo reservado con éxito.';
                $tipo_mensaje = 'alert-success; background-color: #D1FAE5; color: #065F46; border-color: #10B981;';
            } else {
                $mensaje = 'Error al reservar el dispositivo.';
                $tipo_mensaje = 'alert-danger';
            }
            $stmt_insert->close();
        }
...
$res_disp = $conn->query("SELECT id, nombre, codigo, tipo FROM dispositivos WHERE activo = 1 ORDER BY nombre");
?>

<div class="max-w-3xl mx-auto">
    
    <div class="mb-10 text-center sm:text-left">
        <h2 class="text-3xl font-extrabold text-brand-900 tracking-tight">Reservar Dispositivo</h2>
        <p class="mt-2 text-sm text-brand-900/60 font-medium">Selecciona el equipo a solicitar para tu clase (Máx hasta las 21:00).</p>
    </div>

    <?php if ($mensaje && (strpos($tipo_mensaje, 'success') !== false || $tipo_mensaje === 'alert-success')): ?>
        <!-- PANTALLA DE ÉXITO -->
        <div class="bg-white shadow-xl shadow-brand-900/5 ring-1 ring-brand-900/5 rounded-[2.5rem] p-8 sm:p-12 mb-8 max-w-lg mx-auto text-center">
            ...
            <p class="text-brand-900/60 font-medium mb-8 text-sm leading-relaxed">Tu reserva del dispositivo ha sido registrada correctamente en el sistema y ya es visible en el calendario institucional.</p>
    ...
    <?php else: ?>
        ...
        <div class="bg-white shadow-xl shadow-brand-900/5 ring-1 ring-brand-900/5 rounded-[2.5rem] p-8 sm:p-10 mb-8 max-w-2xl mx-auto">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" class="space-y-6">
                ...
                <div>
                    <label for="dispositivo_id" class="block text-xs font-semibold text-gray-500 uppercase tracking-widest pl-4 mb-2">Dispositivo</label>
                    <div class="relative">
                        <select name="dispositivo_id" id="dispositivo_id" class="block w-full px-5 py-4 bg-gray-50/50 border border-gray-200 rounded-full shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-brand-200 focus:border-brand-500 transition-all text-gray-800 appearance-none font-medium" required>
                            <option value="">-- Selecciona Dispositivo --</option>
                            <?php while($disp = $res_disp->fetch_assoc()): ?>
                                <option value="<?php echo $disp['id']; ?>"><?php echo htmlspecialchars($disp['nombre'] . ' [' . $disp['tipo'] . ']'); ?></option>
                            <?php endwhile; ?>
                        </select>
